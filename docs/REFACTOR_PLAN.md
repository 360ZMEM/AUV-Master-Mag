...
2. **perception 直线拟合的结构性失效（✅ 已解决，方案见下）**：`mean_heading_error_deg` 指标测的是 perception 的 `fused_heading`，而非车辆实际航向。在 <50 m 急弯里，`WeightedSlidingWindowFitter` 的**直线**模型无法表达曲率，拟合在弯段冻结，故旧 case5 路由（min r 22.4 m）指标仍显示 22.8°（其余 case 因路线平缓，fused 误差 0.9–2.2° 远小于车辆误差，指标可信）。

   **解决决策（独立估计器路线证伪 → 环境层修正）**：曾按"独立磁法曲率感知中心线估计器"路线实现并验证（声呐缆点弧长窗口 + 切线锚定二次拟合 + 弧垂显著性门控，绝不喂入 PCA 协方差闸门，直缆零回归）。但**实测数据证伪了该路线在本场景的有效性**——逐帧诊断显示估计器在全部 case×mode 下 `valid=0`，从不发火。根因是**物理信噪比**：声呐缆点噪声 rms≈0.42 m，而真实路由曲率太缓（case5 min r 30 m → 13 m 窗口弧垂仅 0.71 m；case6/8 min r 66/78 m → 弧垂 0.32/0.27 m **低于噪声基底**），曲率信号被声呐噪声淹没，弧长加窗平均（√N 降噪）与加长窗口（28–35 m）均无法恢复。结论：在"声呐定位 + 磁法埋深"的传感配置下，无法从声呐几何稳健反演这种缓弯曲率，独立估计器属**不可执行域**，按"高度可维护/无无效点"原则**不予合并**。

   改以**环境层修正**收口（呼应根因 2 修复方向 ③）：放宽 case5 正弦波长（55→62 m、140→150 m），min r 22.4 m → **31.1 m**（≥ 环境设计硬约束 25 m，消除构建期曲率告警）。修正后 case5 fused `mean_err 22.8° → 9.85°`（< 15° 硬约束），TRACK 段车辆实际航向误差仍 0.7°、横偏 0.1 m；case1–4 字节级零回归（C5 守恒）。
