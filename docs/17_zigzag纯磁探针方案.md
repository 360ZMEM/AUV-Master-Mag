...
### 9.6 Lookahead -> local_path 保鲜门控

已实现 `magnetic_lookahead_feed_local_path` 的保鲜门控。即使开启 lookahead 喂入，也必须同时满足：

- `lookahead age` 不超过上限。
- 最近一次相位确认未过期。
- 相对已有 `local_path` 轴线的横向创新不过大。
- 相对已有 `local_path` 轴线的 heading delta 不过大。
- 当前 `local_path` residual 不超过上限。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `p25_probe10_feedlocal_gate60` | 12000 | 25.3 | 7.7m | 41.2° | 37.0% | 31.1m | 41.6% | 17.7m | 25.0% | 60.1% | 门控后短测仍保留 p22 的推进能力。 |
| `p26_probe10_feedlocal_gate90` | 12000 | 25.2 | 7.7m | 41.2° | 37.1% | 39.0m | 41.6% | 17.7m | 25.0% | 60.1% | 放宽 phase/age 与 p25 类似。 |
| `p25_probe10_feedlocal_gate60` | 24000 | 14.5 | 27.0m | 138.8° | 37.6% | 16.2m | 34.7% | 14.1m | 25.0% | 60.1% | 相比未门控 p22 长测回退到 0%，门控能维持推进，但航向仍不可靠。 |
| `p26_probe10_feedlocal_gate90` | 24000 | 20.0 | 24.1m | 69.1° | 39.6% | 15.8m | 38.0% | 25.5m | 34.3% | 47.1% | route 略高，但 lookahead 位置误差更大。 |
| `p27_probe10_gate60_nopursuit` | 24000 | 14.5 | 27.0m | 138.8° | 37.6% | 16.2m | 34.7% | 14.1m | 25.0% | 60.1% | 与 p25 一致，pure-pursuit 不是航向误差来源。 |
| `p28_probe10_gate45_conservative` | 24000 | 43.9 | 8.2m | 63.9° | 9.0% | 0.9m | 20.2% | 3.6m | 34.3% | 47.1% | 保守门控估计最干净，但供给不足，route 低。 |
| `p29_probe10_gate60_mid` | 24000 | 17.0 | 7.7m | 41.2° | 0.0% | 58.7m | 20.8% | 17.7m | 32.7% | 52.1% | 30° heading + 12m innovation 未形成推进。 |
| `p30_probe10_gate75_mid` | 24000 | 17.0 | 7.7m | 41.2° | 0.0% | 58.7m | 20.8% | 17.7m | 32.7% | 52.1% | 与 p29 一致。 |
| `p31_probe10_gate60_heading30` | 24000 | 17.0 | 7.7m | 41.2° | 0.0% | 58.7m | 20.8% | 17.7m | 32.7% | 52.1% | 单独把 heading 阈值设为 30° 仍失败。 |
| `p32_probe10_gate60_heading40` | 24000 | 14.5 | 27.0m | 138.8° | 37.6% | 16.2m | 34.7% | 14.1m | 25.0% | 60.1% | 40° 与 p25 行为一致，说明问题不是简单阈值。 |

额外尝试将 lookahead feed heading 对齐到车辆当前航向，结果 `p25/p28` route 均降到 `0%`，说明当前 local path 更依赖局部轴线连续性，不能简单按车辆航向翻转观测 heading。

阶段判断：保鲜门控已经把“短期推进、长期回退”的 p22 改成“长测可维持 37-40% route”的 p25/p26；但这仍不是可合入默认流程的方案，因为 TRACK vehicle heading error 和 lookahead position error 仍偏大。下一步应围绕 p25/p28 的折中继续：提高相位刷新频率，或把 lookahead feed 分成“高置信相位锚点”和“低权重外推点”，避免外推点以同等权重污染 local path。

### 9.7 门控埋点、关键点复测与可视化增强

已新增逐帧埋点：`magnetic_lookahead_feed_allowed`、`magnetic_lookahead_feed_reason_code`、`magnetic_lookahead_feed_phase_age_s`、`magnetic_lookahead_feed_innovation_m`、`magnetic_lookahead_feed_axis_delta_deg`、`magnetic_lookahead_feed_local_residual_m`。

`reason_code` 编码：`1=allowed`，`2=no lookahead target`，`3=feed disabled`，`4=confidence too low`，`5=lookahead age too large`，`6=phase age too large`，`7=local residual too large`，`8=heading delta too large`，`9=innovation too large`。

`tools/evaluate_dropout_variants.py` 和报告已新增汇总指标：`lookahead_feed_pct`、`feed_reject_age`、`feed_reject_phase_age`、`feed_reject_residual`、`feed_reject_heading`、`feed_reject_innovation`、`feed_phase_age`、`feed_innovation`、`feed_axis_delta`、`feed_local_residual`。

基于新埋点的关键发现：`p25` 与 `p28` 主要被 `heading delta` 卡住，而不是被 innovation 或 residual 卡住。`p25` 长测中 `feed_reject_heading=60.1%`，`p28` 长测中 `feed_reject_heading=47.1%`。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `p31_probe10_gate60_heading30` | 24000 | 17.0 | 7.7m | 41.2° | 0.0% | 58.7m | 32.7% | 52.1% | 30° heading 门控不是有效折中。 |
| `p32_probe10_gate60_heading40` | 24000 | 14.5 | 27.0m | 138.8° | 37.6% | 16.2m | 25.0% | 60.1% | 40° 与 p25 行为一致，说明不是简单阈值问题。 |

详细诊断图 `render_detail()` 已新增：

- Lookahead feed gate reason：随时间显示 `allowed` 与 `reason_code`。
- Lookahead feed gate margins：随时间显示 `axis_delta`、`innovation`、`phase_age`。

下一步计划：

1. 把 lookahead feed 拆成“高置信相位锚点”和“低权重外推点”，避免外推点以同等权重污染 `local_path`。
2. 增加时间同步分析图：`route progress` vs `feed_allowed`，`heading error` vs `reject_heading`，`lookahead position error` vs `local residual`。
3. 若后续形成稳定策略，将上述中间量作为论文图表：可观测性、门控拒绝原因分布、估计器就绪过程和控制收益之间的因果链。
...
`tools/evaluate_dropout_variants.py` 和报告已新增以下汇总指标：
...
报告与 `tools/evaluate_dropout_variants.py` 已输出：

- lookahead feed allowed 占比。
- age / phase age / residual / heading / innovation 各拒绝原因占比。
- phase age、innovation、axis delta、local residual 均值。

详细诊断图 `render_detail()` 已新增两类面板：

- Lookahead feed gate reason：随时间显示 allowed 与 reject reason code。
- Lookahead feed gate margins：随时间显示 axis delta、innovation、phase age。

后续论文/分析图建议：

1. `route progress` vs `lookahead feed allowed` 时间同步图，用于解释推进何时由估计器供给驱动。
2. `heading error` vs `feed reject heading` 时间同步图，用于定位航向误差高是否来自轴线跳变。
3. `lookahead position error` vs `feed local residual`，用于区分观测错误和 local path 自身污染。
4. 将 lookahead feed 拆分为高置信相位锚点与低权重外推点后，单独画两类输入的权重与残差。

## 关键可视化

论文写作和方案复盘建议引用 [23_论文图清单](23_论文图清单.md) 中的以下单面板小图（PNG+PDF 双格式，IEEE 双栏 ~3.5 in）：

| slug | 用途 |
| --- | --- |
| `overview_trajectory` | 展示小幅 zig-zag 探针下的 AUV 轨迹与电缆叠加 |
| `detail_probe_cycle` | 论文方法图：leg sign / phase / X-fwd / X-bwd 一图覆盖 |
| `detail_tracking_strength` | 磁强度与 peak 标记，证明探针带来 crossing 证据 |
| `detail_mag_offset` | (B↓/B⊥)·d 横偏，验证主动激励的几何可解释性 |
| `selector_gate_overlap` | 假设/验证/dual/aligned 门叠加，证明 zig-zag 提供的候选可通过下游 |
