...
1. 先做“磁跨线对齐的 probe 控制”实验，而不是继续调估计器：
   - 在 dropout / magnetic reacquire 状态下，leg flip 除了带宽/看门狗，还应参考 magnetic cross-track sign。
   - 如果磁横偏长期不翻转，则延长当前 leg 或增加 crossing angle，而不是按固定周期翻腿。
   - 输出 `magnetic_crossing_wait_s`、`magnetic_crossing_missed_count`、`magnetic_crossing_forced_flip`。
2. 只有当 `probe_mag_cross_per_cycle` 明显上升后，再进入 D2：
   - 将 cycle summary 变成 explicit hypothesis supply。
   - 保留 `+axis/-axis`，用任务层短窗 selector 选择。
