...
- `magnetic_shadow_validation_max_age_s = 30.0`
...
| variant | route | shadow axis valid | validation pass | reject no hypothesis | reject low score | reject low margin | reject stale age | age over |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `d2_shadow_axis_selector` | 37.0 | 41.6% | 25.1% | 58.4% | 0.1% | 0.0% | 16.3% | 4.2s |

解释：

- 通过帧占全部启用诊断帧的 25.1%，低于 `shadow_axis_valid=41.6%`，主要是部分候选过期。
- 最大拒绝原因仍是无 hypothesis 供给，说明 D3 首要瓶颈仍在 phase/lookahead supply，而不是 score 或 margin。
- 这组日志已经足够支撑下一步 D3 接入前的门控调参：先看 `reason_code=2/6`，再决定是补供给还是放宽 age。
