...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p25_probe10_feedlocal_gate60 --name p28_probe10_gate45_conservative --max-steps 24000
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p33_probe10_tiered_anchor_low --name p34_probe10_anchor_only --name p35_probe10_tiered_anchor_mid --max-steps 24000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --name p37_probe10_extrapolated_mid --max-steps 24000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p39_probe10_extrapolated_low_pursuit
```
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p40_probe10_extrapolated_low_smooth12 --name p41_probe10_extrapolated_low_smooth6 --max-steps 24000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p42_probe10_extrapolated_low_smooth9 --max-steps 24000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p40_probe10_extrapolated_low_smooth12
```
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p43_probe10_extrapolated_low_axis --max-steps 24000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --name p44_probe10_extrapolated_low_curveflip --max-steps 24000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p44_probe10_extrapolated_low_curveflip
```
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p45_probe10_extrapolated_low_axis_hyst --max-steps 24000
```
...
- 早期失败不是单个阈值问题，而是“观测、方向消歧、任务推进、控制消费”被过早压成单一输出。
- 多个方案能降低局部 heading 或横偏，但 route/endpoint 同时退化，说明评估必须以任务级指标为主。
- zig-zag 已能提供激励和诊断窗口，但现有链路还没有把一个完整 probe cycle 转化为可靠的任务层候选。

失败机制表：

| 阶段/方案 | 现象 | 主要失败机制 | 后续处理 |
| --- | --- | --- | --- |
| 直接 magnetic path feed | 容易污染 `local_path` | 单帧磁方向有 180deg 歧义，且无任务层确认 | 保留诊断，禁止默认闭环 |
| phase gate | phase 更干净但稀疏 | 观测太少，不能支撑连续 route 推进 | 用 cycle summary 替代单点 phase |
| lookahead feed p25/p28/p31/p32 | heading gate 不存在稳定折中 | gate 严则供给不足，gate 松则 heading 污染 | 不继续单阈值调参 |
| tiered anchor p33-p35 | route 直接崩 | 高置信稀疏 anchor 主导局部估计 | phase anchor 不作为高权重默认 feed |
| low-weight extrapolated p36 | route 最好但 heading 很差 | 低权重外推能推进，但未解决 axis sign | 保留为 route-progress 对照 |
| heading smoothing p40-p42 | smoothing 强则 route 崩，弱则无效 | heading 内混有真实转弯信息和错误跳变 | 不继续单纯低通 |
| axis selection p43 | route 1.8% | phase-anchor progress 太噪 | 不能单帧/单 phase 提交方向 |
| curve flip p44 | heading 好，full route 40.8% | 控制层翻转改变行为，降低任务推进 | 仅作为方向歧义证据 |
| hysteresis p45 | heading 好，route 0% | builder 内仍过早提交单一方向 | 多假设必须保留到任务层 |
| D2 参数类复测 | 局部指标好但 route 低 | 参数调整不是候选集合 | 需要显式 hypothesis |
| D3 progressive gate | probe 窗口增加但 route 低 | gate 不是任务层 selector | 新增短窗打分器 |
| D4 sparse035 | full route 55.9%，横偏好 | 声呐锚点有效，但策略未超过 p36 | 作为 sparse anchor 正例 |

对下一步的约束：

1. 不再接受“heading/横偏改善但 route 退化”的方案。
2. 不再在感知 builder 内提前删除 `+axis/-axis` 任一假设。
3. 不把 burial/offset 直接闭环，先要求 cycle-level posterior 在 shadow 指标上稳定。
4. sparse sonar 应作为 anchor 校准假设，而不是简单提高检测概率。
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d0 --max-steps 2000
```
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --max-steps 6000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --max-steps 12000
```
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d2 --max-steps 12000
```
...
```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d4 --max-steps 12000
```

- 完整长测命令：

```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d4 --name d4_sparse035 --name d4_sparse050
```
...
- 新增 `MagneticBurialCycleEstimator`：
  - 每个 zig-zag cycle 重置。
  - 只使用 `magnetic_path_observation_valid` 的帧。
  - 对近过线、高 SNR 样本加权。
  - 输出 cycle-local `depth/sigma/quality/sample_count`。
- 新增记录和汇总字段：
  ...
  - `probe_cycle_burial_cov`
  - `probe_cycle_burial_mae`
  - `probe_cycle_burial_sigma`
  - `probe_cycle_burial_quality`

调优验证：

```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --max-steps 12000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d4 --name d4_sparse035 --max-steps 12000
```

| variant | run | probe burial MAE | cycle burial cov | cycle burial MAE | cycle sigma | cycle quality | 结论 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `p36_probe10_extrapolated_low` | 12000 | 1.839m | 0.6% | 0.778m | 0.065m | 0.257 | cycle posterior 明显优于 probe-frame 旧口径，但覆盖仍低。 |
| `d4_sparse035` | 12000 | 1.120m | 0.0% | nan | nan | nan | sparse sonar 不再产生磁 cycle burial 假阳性。 |

单元测试覆盖：

- cycle 内 3-4 个近过线样本即可收敛到真实埋深。
- 远离过线、低 SNR 的污染样本会被降权，不会主导 weighted median。
- cycle reset 后必须重新暖机，避免跨周期泄漏。

当前 D5 结论：

- cycle-level posterior 可以提升 probe 窗口埋深估计质量。
- 供给瓶颈仍然明显：`p36` 的 probe active 只有 1.4%，cycle posterior coverage 只有 0.6%。
- 下一步不应继续调 burial 门控，而应提升有效 probe cycle 数量，并把 cycle summary 接入 D2 hypothesis，而不是闭环控制。

本轮搭配诊断：

- 新增 shadow hypothesis readiness 诊断：
...
- 新增 probe/磁几何对齐诊断：
  ...
  - `probe_mag_crossings`
  - `probe_mag_cross_per_cycle`
- `probe_active` 口径从仅 `track` 扩展为 `align/track`。原因是当前控制器在 `LOCK_ALIGN`
  也会保持 zig-zag legs，且 `p36` 的大部分磁观测发生在 `align`。

验证命令：

```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --max-steps 12000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p46_probe10_relaxed_phase_supply --max-steps 12000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d4 --name d4_sparse035 --max-steps 12000
```

| variant | run | probe active | probe cycles | magnetic crossings | crossings / cycle | phase / cycle | shadow supply | shadow selection | shadow consumption | bottleneck supply | route | 结论 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `p36_probe10_extrapolated_low` | 12000 | 66.4% | 36 | 6 | 0.17 | 0.17 | 0.356 | 0.450 | 0.665 | 97.1% | 37.0% | 有大量 probe leg cycle，但磁跨线不足。 |
| `p46_probe10_relaxed_phase_supply` | 12000 | 69.9% | 38 | 6 | 0.16 | 0.21 | 0.356 | 0.450 | 0.665 | 97.1% | 36.5% | 放宽 phase 条件不能解决供给瓶颈。 |
| `d4_sparse035` | 12000 | 68.2% | 55 | 0 | 0.00 | 0.00 | 0.000 | 0.429 | 0.650 | 100.0% | 19.4% | sparse sonar 是锚点正例，但无纯磁 hypothesis。 |

解释：

- 当前不是 burial 估计器精度主导，也不是 phase 阈值太严。
- 主要问题是控制层的 zig-zag leg cycle 没有稳定转化为磁横偏跨线事件。
- `p36` route 能推进，是因为低权重 lookahead feed 和已有控制逻辑偶然维持了部分路线；
  但 shadow readiness 显示 hypothesis supply 长期不足，因此后续直接做任务层 selector 还缺候选。

下一步行动：

1. 磁跨线对齐的 probe 控制作为实验开关保留，但不进入默认主路径：
   - 已实现默认关闭的 `magnetic_crossing_probe_control_enabled`。
   - 已输出 `magnetic_crossing_probe_wait_s`、`magnetic_crossing_probe_missed_count`、`magnetic_crossing_probe_forced_flip`。
   - 代表点评估显示它没有提高 `probe_mag_cross_per_cycle`，因此不能作为 D2 前置条件。
2. 下一步进入 D2，但 D2 必须保持旁路 selector：
   - 控制层继续以 p36 小幅 probe + lookahead feed 作为基线。
   - 将 cycle summary 变成 explicit shadow hypothesis supply。
   - 保留 `+axis/-axis`，用任务层短窗 route/control progress 选择，不让单帧磁符号直接抢控制。
3. D4 sparse sonar 继续作为 anchor 正例：
   - sonar 命中用于校准候选，不直接替代纯磁 hypothesis。

### p47：磁跨线对齐 probe 控制实验

目的：验证“让 leg flip 参考磁横偏符号/过零”能否把 p36 中大量 probe cycle 转化为更多有效磁跨线事件。

实现：

- `TrackingConfig` 新增默认关闭开关：
  - `magnetic_crossing_probe_control_enabled`
  - `magnetic_crossing_probe_min_abs_offset_m`
  - `magnetic_crossing_probe_min_flip_interval_s`
  - `magnetic_crossing_probe_forced_flip_multiplier`
  - `magnetic_crossing_probe_max_wait_s`
- controller 优先使用连续的 `magnetic_cross_track_offset_m` 判断符号；
  缺失时退回 `magnetic_path_cross_track_offset_m`，并按 `magnetic_path_heading_deg`
  与 controller base heading 的 180° 关系归一化符号。
- 加入最小翻腿间隔，避免 ratio/path 符号抖动造成高频 leg flip。

验证命令：

```bash
/Users/auv_user/miniconda3/bin/python -m unittest tests.test_controller_reacquire tests.test_viz_progress
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p36_probe10_extrapolated_low --max-steps 12000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase probe --name p47_probe10_magnetic_crossing_control --max-steps 12000
```

| variant | route | probe cycles | magnetic crossings | crossings / cycle | forced flips | missed crossings | lookahead feed | shadow supply | 结论 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `p36_probe10_extrapolated_low` | 37.0 | 36 | 6 | 0.17 | 0 | 0 | 33.3% | 0.356 | 当前基线，crossing 少但 route 仍可推进。 |
| `p47_probe10_magnetic_crossing_control` | 2.3 | 46 | 2 | 0.04 | 1 | 1 | 0.0% | 0.583 | 控制层磁符号对齐未增加 crossing，且破坏 feed/route。 |

中间失败形态：

- 直接用 path 符号等待 crossing：`0.00 crossings/cycle`，大量 forced flip，说明会卡在同侧。
- 加 heading 归一化后：单帧 heading 180° 抖动导致 893 个 cycle，说明不能让单帧磁 path 符号直接驱动控制。
- 改为 ratio 优先 + 10s 最小间隔后，高频翻腿被抑制，但 `crossings/cycle` 仍低于 p36，route 明显退化。

结论：

- 问题不是“缺一个更聪明的 leg flip 条件”这么简单。
- 磁横偏符号在 maze dropout 中仍受 heading 方向、分支和局部几何影响，直接闭环到控制会放大错误。
- p47 负结果保留在本文档中，但不再作为评估脚本中的维护样本；后续不继续沿控制层磁符号对齐方向调参。

### D2：旁路多假设 axis selector

目的：在不改控制、不喂 local_path 的前提下，验证“保留 `+axis/-axis` 到任务层短窗选择”是否能产生稳定候选。

实现：

- 新增 `MagneticShadowHypothesisSelector`：
  - 只消费 phase anchor、车辆当前位置和车辆航向。
  - 每个 phase anchor 同时保留 `+axis/-axis` 两个候选。
  - 用短窗车辆推进方向和当前 heading 对两个候选打分。
  - 输出 selected sign、score、margin、target 和 age。
- 新增默认关闭配置：
  - `magnetic_shadow_hypothesis_enabled`
  - `magnetic_shadow_hypothesis_min_progress_m`
- 新增记录/汇总字段：
  - `shadow_axis_hypothesis_valid`
...
| variant | route | probe cycles | magnetic crossings / cycle | shadow axis valid | shadow axis score | shadow axis margin | lookahead feed | 结论 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `p36_probe10_extrapolated_low` | 37.0 | 36 | 0.17 | 0.0% | nan | nan | 33.3% | 当前控制/feeding 基线。 |
| `d2_shadow_axis_selector` | 37.0 | 36 | 0.17 | 41.6% | 0.897 | 0.733 | 33.3% | 旁路 selector 不扰动 route，并能在 phase/lookahead 有供给窗口内给出高 margin 方向选择。 |

结论：

- D2 旁路 selector 是正结果：它没有改善 route，因为还没有接入控制，但证明了 `+axis/-axis`
  不应在 builder 内提前坍缩；保留到任务层后，短窗推进能给出可观测的选择 margin。
- 当前供给窗口仍只有 41.6%，所以直接闭环还太早。
- 下一步进入 D3：把 `shadow_axis_score_margin` 和 `shadow_axis_age_s` 作为任务层 selector 门控，
  只在高 margin、低 age、lookahead feed 通过或 local path residual 允许时，才允许候选影响目标。

### D3：影子验证日志

目的：在 D3 真正接入控制前，先把“为什么通过/为什么拒绝”记录清楚，避免 selector 只给一个总分却无法定位瓶颈。

新增默认配置：

- `magnetic_shadow_validation_min_score = 0.70`
- `magnetic_shadow_validation_min_margin = 0.25`
- `magnetic_shadow_validation_max_age_s = 45.0`

新增逐帧日志字段：

- `shadow_axis_validation_passed`
- `shadow_axis_validation_reason_code`
- `shadow_axis_validation_score_deficit`
- `shadow_axis_validation_margin_deficit`
- `shadow_axis_validation_age_over_s`

`shadow_axis_validation_reason_code` 含义：
...
| 2 | 无 hypothesis 供给 |
| 3 | 候选数量不足 |
| 4 | selector score 低于门槛 |
| 5 | `+axis/-axis` 分差低于门槛 |
| 6 | hypothesis 超过 validation age |
| 7 | selector anchor 超过 selector 保留窗口 |

代表点结果：

```bash
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d2 --name d2_shadow_axis_selector --max-steps 12000
/Users/auv_user/miniconda3/bin/python tools/evaluate_dropout_variants.py --phase d3 --name d3_shadow_axis_selector_age45 --max-steps 12000
```

| variant | validation age | route | shadow axis valid | validation pass | reject no hypothesis | reject validation age | reject selector expired | age over |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `d2_shadow_axis_selector` | 30s | 37.0 | 41.6% | 25.1% | 56.0% | 16.3% | 2.5% | 4.4s |
| `d3_shadow_axis_selector_age45` | 45s | 37.0 | 41.6% | 30.1% | 56.0% | 11.3% | 2.5% | 2.3s |

解释：

- 将 validation age 从 30s 放宽到 45s 后，pass 从 25.1% 提高到 30.1%，route 保持 37.0。
- 过期拒绝从 16.3% 降到 11.3%，说明 age 门控确实是第二瓶颈；但最大拒绝原因仍是无 hypothesis 供给。
- `reason_code=7` 仅 2.5%，说明 selector 保留窗口不是主瓶颈；后续更应补 phase/lookahead 供给，而不是继续大幅放宽 selector max age。

### D3：四类瓶颈聚合与双门控影子记录

为后续 D3 接入控制做铺垫，本轮把逐 reason code 进一步聚合为四类，并新增一个完全影子的双门控（selector validation + lookahead feed）记录开关 `magnetic_shadow_dual_gate_shadow_enabled`，默认关闭，不影响控制。聚合定义：

- `shadow_axis_supply_fraction` = reject `2 + 3`（无 anchor 或候选不足）。
- `shadow_axis_validation_fraction` = reject `6 + 7`（age 类）。
- `shadow_axis_selection_fraction` = reject `4 + 5`（分数 / margin）。
- `shadow_axis_consumption_fraction` = `validation pass × dual_gate.reject_feed_rate`，即若 D3 接入控制将被消费层拒绝的比例。

对照评估（`case_maze_sonar_dropout`，max_steps=12000）：

| variant | route | supply% | validation% | selection% | consumption% | dual pass% | dual reject feed% |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `d2_shadow_axis_selector` | 37.0 | 56.0 | 18.8 | 0.1 | 0.0 | n/a | n/a |
| `d3_shadow_axis_selector_age45` | 37.0 | 56.0 | 13.8 | 0.1 | 0.0 | n/a | n/a |
| `d3_shadow_axis_dual_gate_shadow` | 37.0 | 56.0 | 13.8 | 0.1 | 4.6 | 14.8 | 15.3 |

结论：

- selection 维度的拒绝继续证伪了"调 score/margin 阈值有用"。
- 即使 selector validation 通过，lookahead feed 仍会过滤掉一部分候选；若直接接入控制，dual gate pass 只有 14.8%。
- 这说明 D3 接入前还需先消化 supply 与 feed 两个外部约束，再考虑是否把 shadow_axis 接入控制。

下一步建议：

1. 优先提升 `probe_phase_per_cycle`，把 supply% 从 56% 降下来。
2. 在做控制层闭环前，再用 `shadow_axis_dual_gate_shadow` 影子复跑，确认 dual pass% 显著提升后才接入。
3. 不建议立刻继续放宽 validation age 或 score/margin 阈值，因为 selection 维度几乎为零，再放宽只会放噪声。

### D3：死分支剪枝与 phase detector 供给诊断

本轮按上述建议继续先处理 supply 瓶颈，同时对已经证伪的尝试做评估入口剪枝。

剪枝范围：

- 从 `tools/evaluate_dropout_variants.py` 删除 p40-p47 代表点：
  - `p40`/`p41`/`p42`：heading smoothing。
  - `p43`：phase-progress 单向 axis selection。
  - `p44`：controller curve flip。
  - `p45`：builder 内 hysteresis axis selection。
  - `p46`：relaxed phase supply。
  - `p47`：magnetic-crossing probe control。
- 历史负结果继续保留在本文档中，作为后续设计约束；但不再让这些分支进入日常评估和基线维护。

额外供给实验：

- 临时验证 `d3_shadow_supply_phase_window90`：只把 phase detector 最大周期窗口放宽到 90s。
- 结果：`route=0.0%`，`probe_phase_per_cycle=0.21`，`shadow_axis_supply_pct=56.0%`。
- 结论：单纯扩大 phase 时间窗不能解决供给瓶颈，且会破坏 route progress；该实验也已从评估脚本删除。

因此本轮转为补充 `MagneticZigzagPhaseDetector` 的拒绝原因日志。新增逐帧诊断：
...
| ---: | --- |
| 0 | 未启用 / 无诊断 |
| 1 | 成功输出 phase observation |
| 2 | cross-track offset 为 0，无法确定符号 |
| 3 | 初始化第一侧符号，等待 crossing |
| 4 | 符号刚切换，记录当前侧极值，等待形成稳定 pair |
| 5 | 缺少 previous/current extreme pair |
| 6 | 当前极值 offset 低于 `min_offset_m` |
| 7 | candidate duration 过短 |
| 8 | candidate duration 过长 |
| 9 | previous/current axis delta 过大 |
| 10 | 正常等待中，尚未触发明确拒绝 |
...
| variant | route | phase/cycle | supply% | validation% | selection% | consumption% | dual pass% | phase emit | no-pair | offset reject | duration reject | axis reject | candidate duration | axis delta |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `d3_shadow_axis_dual_gate_shadow` | 37.0 | 0.17 | 56.0 | 13.8 | 0.1 | 4.6 | 14.8 | 0.1% | 42.3% | 0.0% | 49.1% | 8.5% | 5.2s | 64.1deg |

解释：

- `phase_offset_reject_pct=0.0%`，说明 offset 门控不是当前供给瓶颈。
- 拒绝主要集中在 no-pair 与 duration：有效 crossing pair 没有稳定形成，或形成后的周期不满足 phase detector 时间约束。
- `phase_axis_delta=64.1deg` 说明极值 pair 之间的磁路径轴线仍明显不一致，但 axis reject 不是最大项。
- 下一步应聚焦 duration/no-pair/axis-consistency 的形成机制，而不是继续调 score/margin、validation age 或单纯扩大 phase 时间窗。
